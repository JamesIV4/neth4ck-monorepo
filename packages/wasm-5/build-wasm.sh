#!/bin/bash
# Build NetHack 5.0 for WebAssembly (WASM) using Emscripten.
#
# This keeps the 5.0 CROSS_TO_WASM flow, but mirrors the 3.6.7 staged build:
...
if [ ! -f "$NH/sys/unix/setup.sh" ]; then
    ...
    echo "  Run: git submodule update --init packages/wasm-5/NetHack"
    exit 1
fi

echo "=== NetHack 5.0 WASM Build ==="
echo "Source: $NH"
echo ""
...
cd "$NH"
if ! (cd sys/unix && sh ./setup.sh hints/linux.500); then
...
# ------------------------------------------------------------------
# Fetch Lua dependency (required for NetHack 5.0)
# ------------------------------------------------------------------
echo ""
echo "--- Fetching Lua ---"
make -C "$NH" fetch-lua
...
# NetHack 5.0 does not expose lev_comp and dgn_comp as standalone utility targets.
...
# NetHack 5.0's dat/all does not include the 'options' target, but nhdat packaging expects it.
