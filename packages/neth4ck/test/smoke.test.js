...
describe.each([
    ...
    },
    {
        ...
        expectsQuestIntroText: false,
    },
])(
    "integration with $label",
    ({
        version,
        playerSelectionCallback,
        minNhdatSize,
        requiredFsFiles,
        requiredInitCallbacks,
        expectsQuestIntroText,
    }) => {
        let result;
        ...
        describe("callback system", () => {
            ...
            it.each(requiredInitCallbacks)("fires %s callback", (name) => {
        ...
        describe("argument decoding via decodeArgs", () => {
            ...
            it.skipIf(!expectsQuestIntroText)("decodes shim_putstr with window name and text content", () => {
            ...
            it.skipIf(!expectsQuestIntroText)("receives quest intro text via shim_putstr", () => {
        ...
        describe("NETHACKOPTIONS passthrough", () => {
            it("sets NETHACKOPTIONS in the WASM ENV when provided", async () => {
                const r = await runGame(version, ["--nethack-options=name:SmokeHero,autoquiver,!checkpoint"]);
                expect(r.envNethackOptions).toBe("name:SmokeHero,autoquiver,!checkpoint");
